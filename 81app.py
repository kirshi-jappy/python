...
def get_total_usage():
    """API to get total usage summary grouped by zone, quarterly, and optionally filtered by customer name."""
    ...
    if customer_filter:
        filtered_data = filtered_data[filtered_data['customer'] == customer_filter]
    if zone_filter:
        filtered_data = filtered_data[filtered_data['zone'] == zone_filter]
    if quarterly_filter:
        filtered_data = filtered_data[filtered_data['quarterly'] == quarterly_filter]

    # Ensure that the 'formatted_date' column exists in the filtered data
    filtered_data['formatted_date'] = filtered_data['date'].dt.strftime('%b, %Y')
    ...
    # Group by customer, zone, quarterly, and formatted date, then calculate total usage
    result = (
        filtered_data.groupby(['customer', 'zone', 'quarterly', 'formatted_date', 'year', 'month'])['usage']
        .sum()
        .reset_index()
        .rename(columns={'usage': 'Total usage'})
    )
    ...
    result = result.sort_values(by=['year', 'month']).drop(columns=['year', 'month'])
